...
// 读取原始文件
const filePath = path.join(__dirname, '../src/ui/testUIHtml.ts');
let content = fs.readFileSync(filePath, 'utf8');

// 1. 在 App 组件中添加请求历史状态
const appStateRegex = /const \[templatesData, setTemplatesData\] = useState\(\{\}\);/;
const requestsHistoryState = `
            const [templatesData, setTemplatesData] = useState({});
            const [requestsHistory, setRequestsHistory] = useState([]);
            const requestHistoryRef = useRef([]);`;

content = content.replace(appStateRegex, requestsHistoryState);

// 2. 修改 showResponse 函数
const showResponseRegex = /\/\/ Show response\s*\n\s*const showResponse = \(data, status\) => \{[\s\S]*?\};/;
const modifiedShowResponse = `// Show response and record in history
...
content = content.replace(showResponseRegex, modifiedShowResponse);

// 3. 添加 DebugConsole 组件（在 ChannelSelector 组件后）
const channelSelectorEndRegex = /}\s*\/\/ Quick Fill Buttons Component/;
...
content = content.replace(channelSelectorEndRegex, debugConsoleComponent);

// 4. 在 App 组件的 return 中添加 DebugConsole
const toastRegex = /{\\/\\* Toast Notification \\*\\/}/;
const addDebugConsole = `{/* Debug Console */}
                    <DebugConsole 
...
content = content.replace(toastRegex, addDebugConsole);

// 写回文件
fs.writeFileSync(filePath, content, 'utf8');

console.log('修改已应用到 testUIHtml.ts 文件');